...
UI_PATH = os.path.join(PUBLIC_DIR, "index.html")
SCORECARD_PATH = os.path.join(PUBLIC_DIR, "scorecard.html")
...
def scorecard() -> FileResponse:
    """Serve the OWASP scorecard, linked from the intro banner on the UI.

    Vercel serves anything under public/ as a static asset automatically, so
    this route exists only so the SAME link works identically in local dev,
    where FastAPI has no such automatic behaviour -- it only serves what it's
    explicitly given a route for. One snapshot file, committed deliberately
    (unlike reports/scorecard.html, which is gitignored and regenerated by
    report/generate_scorecard.py); refresh public/scorecard.html by hand after
    a real red-team run if the findings change.
    """
    return FileResponse(SCORECARD_PATH)
